...
internal class Program
{
    ...
    // Class for dictionaries in Ukrainian (to change the language, just use another class and constructor)
    static BotMessagesEnglish botMessages = new BotMessagesEnglish();

    // Class for topics in Ukrainian (to change the language, just use another class and constructor)
    ...
    private static async void HandleCallbackQuery(ITelegramBotClient client, CallbackQuery callbackQuery)
    {
        ...
        else if (callbackQuery.Data.StartsWith("user"))
        {
            ...
            {
                if (votes[callbackQuery.Message.Chat.Id].ContainsKey(callbackQuery.Data.Split("_")[1]))
                    votes[callbackQuery.Message.Chat.Id][callbackQuery.Data.Split("_")[1]]++;
                
                else
                    votes[callbackQuery.Message.Chat.Id][callbackQuery.Data.Split("_")[1]] = 1;

                isVoted[callbackQuery.Message.Chat.Id][callbackQuery.From.Id] = true;
                await client.SendTextMessageAsync(callbackQuery.Message.Chat.Id, $"@{callbackQuery.From.Username}" + botMessages.textMessages["votingChose"] + $"@{callbackQuery.Data.Split("_")[1]}");
                // Using sleep to not make too many requests
                Thread.Sleep(requestDelay);
        ...
        else switch (callbackQuery.Data)
        {
            case "save_user":
                ...
                if (!usersIds[callbackQuery.Message.Chat.Id][0].Contains($"{callbackQuery.From.Id}") && usersIds[callbackQuery.Message.Chat.Id][0].Count() < 10) 
                {
                    // Saving user ID
                    usersIds[callbackQuery.Message.Chat.Id][0] = usersIds[callbackQuery.Message.Chat.Id][0].Append<string>($"{callbackQuery.From.Id}").ToList();

                    // Saving username
                    usersIds[callbackQuery.Message.Chat.Id][1] = usersIds[callbackQuery.Message.Chat.Id][1].Append<string>($"{callbackQuery.From.Username}").ToList();
